...
def write_plc(plc: LogixDriver, machine_num: str, results: dict) -> None:
    ...
    try:
        ...
        print_color(f"KeyError in write_plc: {key_error}")
    except Exception as error:
        print_color(f"An error occurred while writing to the PLC: {error}")
# END write_plc
...
def write_plc_fault(plc:LogixDriver,machine_num:str,fault_type:int)->None:
    if fault_type == 2:
        print_color(f'({machine_num}) Trigger Keyence timeout fault (longer than 3 seconds)! PhoenixFltCode : 2')
        ...
        print_color(f'({machine_num}) TIMEOUT ERROR: Keyence not running (Execution time exceeded 3 seconds)\n\tPhoenixFltCode : 3\n')
