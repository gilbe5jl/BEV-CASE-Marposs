...
def print_red(message:str)->None:
    ...
    print(Fore.RED + f"{message}" + Style.RESET_ALL,end='\r')
...
def stage_zero_preLoad(machine_num:str,plc:LogixDriver,sock:socket.socket,config_info)->int:
    ...
    while True:
        ...
        print_red(f"PLC(TAG DATA): {tag_data}")
        ...
        if int(part_program) != 0:
            print_color(f"({machine_num}) PLC(PART_PROGRAM, {part_program})...\n")
            ...
            if (counter % 200) == 0: print_red(f"({machine_num})[STAGE:0] Reading PART_PROGRAM as: ({part_program}), ERROR: Incorrrect PART_PROGRAM. Attempting to retrive PART_PROGRAM...")
            counter += 1
            time.sleep(0.001)
def stage_zero_load(plc: LogixDriver, sock: socket.socket, machine_num: str, tag_data: dict,keyence_string) -> None:
    try:
        ...
        part_prog =  str(tag_data[config_info['tags']['PartProgram']][1])
        print_color(f'({machine_num})[STAGE:0] LOADING KEYENCE: Part Program ({part_prog}), Keyence String: ({keyence_string})...\n')
        ...
        print_color(f'({machine_num}) Error in utils - stage_zero_load: {error}')
        raise error
