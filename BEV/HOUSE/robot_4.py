...
def read_config()->dict:
    with open(os.path.join(sys.path[0], f'config_r{MACHINE_NUMBER}.json'), "r") as config_file:
...
class create_cycle:
    ...
    def cycle(self):
        ...
        with LogixDriver(config_info['plc_ip']) as plc: #context manager for plc connection, currently resetting connection after ~24 hrs to avoid issues
            ...
            while(True):
                if(kill_threads.is_set() ): log_print(f'({machine_num})[PRE-STAGE:0] RESET DETECTED\n') ; break #check reset at beginning of cycle
                ...
                if(self.current_stage == 0):
                    ...
                    while(tag_data[config_info['tags']['LoadProgram']][1] != True): #Looping until LOAD PROGRAM goes high  # Data from PLC is only valid while LOAD_PROGRAM is low
                        if (kill_threads.is_set()): log_print(f'({machine_num})[STAGE:0] RESET DETECTED\n') ; break #check for reset at beginning of cycle
                    ...
                    if swap_check == 0: kill_threads.set() # reset threads if invalid part type
                    part_type,keyence_string = stage_zero_data(machine_num, tag_data, sock, config_info,part_program).values() #building out external Keyence string for scan file naming
                ...
                #################### STAGE ONE ####################
                    ...
                    log_print(f'({machine_num})[STAGE:1] Waiting for START_PROGRAM...\n')
                    while not tag_data[config_info['tags']['StartProgram']][1]: #looping until PLC(START_PROGRAM) goes high
                        if (kill_threads.is_set()): log_print(f'({machine_num})[STAGE:1] RESET detected while waiting for START_PROGRAM\n') ; break #check for reset at beginning of cycle
                ...
                #################### END STAGE ONE ####################
                    ...
                    stage_two_start(plc, machine_num)
                    while not tag_data[config_info['tags']['EndProgram']][1]:
                        if kill_threads.is_set(): log_print(f'({machine_num})[STAGE:2] RESET DETECTED...\n') ; break
                        tag_data = read_plc_dict(plc, machine_num)  # continuous PLC read
                        if tag_data[config_info['tags']['Reset']][1]:log_print(f'({machine_num})[STAGE:2] RESET DETECTED...\n') ; self.current_stage = reset_plc_tags(plc, machine_num, 'type_two') ; kill_threads.set() # type_two reset for stage 2
                ...
                if abs(datetime.datetime.now() - connection_timer).total_seconds() > 86400: log_print(f'({machine_num})[STAGE:2] Connection RESET DETECTED...\n') ; connection_timer = datetime.datetime.now() ; kill_threads.set() # if connected for 24 hours perform restart
...
def heartbeat(machine_num): #sets PLC(Heartbeat) high every second to verify we're still running and communicating
    ...
    def write_heartbeat(plc, machine_num):
        try:
            ...
            log_print(f'({machine_num}-HB) Timeout in Heartbeat {error}')
            log_print("Failed to establish PLC connection after multiple retries.")
            kill_threads.set()
    with LogixDriver(config_info['plc_ip']) as plc:
        log_print(f'({machine_num}) Heartbeat connected to PLC.\n')
        # while (True):
        counter = 0
        while not (kill_threads.is_set()):
            ...
            counter += 1
            sleep_time.sleep(1)
        log_print(f'({machine_num}-HB) Heartbeat: kill_threads high or reset event set, restarting all threads')
        kill_threads.clear()
